  ...
  return (
    <div className="flex w-full justify-center items-center gap-4 py-4 px-6 mb-4 touch-none">
      <div className="flex items-center gap-3 bg-white px-6 py-4 rounded-full shadow-[0_4px_24px_-8px_rgba(0,0,0,0.12)] border border-gray-100">
        {operators.map((op) => (
          <DraggableItem key={op} operator={op} />
        ))}
      </div>
    </div>
  );
...
function DraggableItem({ operator }: { operator: InlineOperator }) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`flex h-12 w-12 cursor-grab active:cursor-grabbing items-center justify-center rounded-full bg-gray-50 text-2xl font-light text-gray-800 transition-all hover:scale-110 hover:bg-gray-100 hover:shadow-sm ${
        isDragging ? 'opacity-0' : 'opacity-100'
      }`}
      aria-label={`${operator} 연산자 드래그`}
    >
      {operator}
    </div>
  );
