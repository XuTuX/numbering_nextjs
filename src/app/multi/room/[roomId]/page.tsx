...
import MultiNumberingEditor from '@/components/MultiNumberingEditor';
...
export default function RoomPage() {
  ...
  const [status, setStatus] = useState<'LOBBY' | 'PLAYING' | 'ROUND_END' | 'GAME_END'>('LOBBY');
  ...
  useEffect(() => {
    ...
    socket.on('round_started', (data: any) => {
      ...
      setDigitString(data.digitString);
    });
    ...
    return (
      <div className="min-h-[100dvh] bg-[#FAFAFA] flex flex-col items-center justify-center p-4">
        <div className="absolute top-8 left-8">
          <Link href="/multi" className="text-[#8A8A8A] hover:text-[#111111] transition-colors text-sm">
            ← Leave
          </Link>
        </div>
    ...
    return (
      <div className="min-h-[100dvh] bg-[#FAFAFA] flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-md bg-[#111111] p-10 rounded-3xl shadow-xl flex flex-col items-center animate-in zoom-in-95 duration-500">
          ...
          <Link href="/multi" className="w-full py-4 rounded-2xl bg-white text-[#111111] text-center font-medium hover:bg-[#FAFAFA] transition-colors active:scale-[0.98]">
            로비로 돌아가기
          </Link>
        </div>
      </div>
