...
수식 공방 친선전에서는 각 라운드 동안 방 전체가 발견한 수식을 공유합니다. 같은 수식은 가장 먼저 제출한 플레이어만 점수를 얻습니다.

메인 화면의 **대전 모드**에서는 두 명이 닉네임으로 같은 방에 입장합니다. 5분 동안 수식 공방, 수열 탐정, 숫자 금고가 무작위 순서로 반복 출제됩니다. 두 플레이어가 같은 문제를 동시에 풀며, 먼저 정답을 맞힌 플레이어만 1점을 얻고 양쪽 모두 다음 문제로 넘어갑니다.
